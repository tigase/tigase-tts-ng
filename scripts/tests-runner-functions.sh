...
function get_database_uri() {
    ...
    case ${_db_type} in
        ...
        derby)
            ...
            ;;
        mongodb)
            [[ -z ${_db_user} ]] && export JDBC_URI="mongodb://${_database_host}/${_db_name}?uuidrepresentation=standard" ||  export JDBC_URI="mongodb://${_db_user}:${_db_pass}@${_database_host}/${_db_name}?uuidrepresentation=standard"
            ;;
        *)
...
function run_test() {
  ...
  echo "Runner function JVM options: ${JAVA_TOOL_OPTIONS}"
