...
public class TestMessageArchivingTagsAndContains extends AbstractTest {
	...
	public void testStorageAndQueryWithTags() throws Exception {
		...
		for (String msg : expectedMessages) {
			assertTrue(mutex.isItemNotified("2:" + id + ":retriveCollection:success:" + msg), "Not retrieved message which was marked by hashtag " + tagName);
		}
		for (String msg : forbiddenMessages) {
			assertFalse(mutex.isItemNotified("2:" + id + ":retriveCollection:success:" + msg), "Retrieved message which was not marked by hashtag " + tagName);
		}
	}
	...
	public void testTagsSuggestions() throws Exception {
		...
		for (String tag : goodTags) {
			assertTrue(mutex.isItemNotified("3:" + id + ":queryTags:success:" + tag), "Not returned tag '" + tag +  "' which should be suggested");
		}
		for (String tag : badTags) {
			assertFalse(mutex.isItemNotified("3:" + id + ":queryTags:success:" + tag), "Returned tag '" + tag + "' which should not be suggested");
		}
	}
	...
	private void retrieveArchivedCollections( final Jaxmpp jaxmppUser1, final String id, final Criteria crit) throws JaxmppException, InterruptedException {
	...
	private void retrieveArchivedMessages(final Jaxmpp jaxmppUser1, final String id, final Criteria crit) throws JaxmppException, InterruptedException {
		jaxmppUser1.getModule(MessageArchivingModule.class)
