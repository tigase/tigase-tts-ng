 ...
 * @author andrzej
 */
public class TestOfflineMessagesLimit
	...
	@Test(groups = {
			"XMPP - Offline Messages"}, description = "Setting offline messages limit to 0 - disabling offline storage")
	...
	private void setLimit(final Mutex mutex, Jaxmpp jaxmpp, final int limit)
		...
		jaxmpp.send(iq, new AsyncCallback() {
			...
			public void onError(Stanza responseStanza, XMPPException.ErrorCondition error) throws JaxmppException {
				mutex.notify("offline:limit:set:null:" + responseStanza.getAttribute("type") + ":" +
									 responseStanza.getAttribute("id"));
			}

			@Override
			public void onSuccess(Stanza responseStanza) throws JaxmppException {
				mutex.notify(
						"offline:limit:set:" + responseStanza.getFirstChild("msgoffline").getAttribute("limit") + ":" +
	...
	private void testLimit(final Mutex mutex, Jaxmpp sender, Jaxmpp receiver, int limit)
			throws JaxmppException, InterruptedException {
				...
				.addHandler(MessageModule.MessageReceivedHandler.MessageReceivedEvent.class,
							new MessageModule.MessageReceivedHandler() {
								...
								public void onMessageReceived(SessionObject sessionObject, Chat chat, Message stanza) {
									try {
										mutex.notify(
												"offline:message:error:" + stanza.getFirstChild("body").getValue());
		...
		for (int i = 0; i < 10; i++) {
			...
			sender.getModule(MessageModule.class)
					.sendMessage(JID.jidInstance(receiver.getSessionObject().getUserBareJid()), "test", body);
			sent.add(body);
			Thread.sleep(100);
		}

		receiver.getEventBus()
				.addHandler(MessageModule.MessageReceivedHandler.MessageReceivedEvent.class,
							new MessageModule.MessageReceivedHandler() {
								...
								public void onMessageReceived(SessionObject sessionObject, Chat chat, Message stanza) {
									try {
										mutex.notify(
												"offline:message:success:" + stanza.getFirstChild("body").getValue());
									} catch (XMLException ex) {
										Logger.getLogger(TestOfflineMessagesLimit.class.getName())
												.log(Level.SEVERE, null, ex);
									}
