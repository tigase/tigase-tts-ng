/*
 * TestRetrievalOfUserAvatarUsingREST.java
 *
 * Tigase TTS-NG
 ...
 * @author andrzej
 */
public class TestRetrievalOfUserAvatarUsingREST
	...
	public void testUserAvatarSupportXEP0084()
			throws JaxmppException, InterruptedException, XMLException, NoSuchAlgorithmException, MalformedURLException,
				   IOException {
	...
	private void publishAvatar(final Mutex mutex, Jaxmpp jaxmpp1, byte[] data)
			throws XMLException, JaxmppException, NoSuchAlgorithmException, InterruptedException {
		...
		pubsubModule.publishItem(jaxmpp1.getSessionObject().getUserBareJid(), USER_AVATAR_DATA_NODE, id, dataPayload,
								 new PubSubModule.PublishAsyncCallback() {
									 ...
									 public void onPublish(String itemId) {
										 assertEquals("Received confirmation of publication of item with wrong id", id,
													  itemId);
									 ...
									 protected void onEror(IQ response, XMPPException.ErrorCondition errorCondition,
										 ...
										 fail("Was not able to publish data: " + errorCondition + " - " +
		...
		pubsubModule.publishItem(jaxmpp1.getSessionObject().getUserBareJid(), USER_AVATAR_METADATA_NODE, id,
								 metaPayload, new PubSubModule.PublishAsyncCallback() {
	...
	private void retrieveAvatar(final Mutex mutex, final Jaxmpp jaxmpp, BareJID userJid, final byte[] data,
								final boolean shouldReceive, final String uuid)
			throws JaxmppException, NoSuchAlgorithmException, InterruptedException {
		PubSubModule pubsubModule = jaxmpp.getModule(PubSubModule.class);

		final String id = hash(data);
		pubsubModule.retrieveItems(userJid, USER_AVATAR_METADATA_NODE, 1, null,
								   new PubSubModule.RetrieveItemsAsyncCallback() {
									   ...
									   protected void onEror(IQ response, XMPPException.ErrorCondition errorCondition,
											   ...
											   throws JaxmppException {
										   if (!shouldReceive) {
											   mutex.notify("received:meta:" + id + ":" + jaxmpp.getSessionObject()
													   .getUserBareJid()
													   .toString() + ":" + uuid);
										   } else {
											   fail("Should receive meta result but got error = " + errorCondition +
															", " + pubSubErrorCondition);
										   }
									   }
									   ...
									   protected void onRetrieve(IQ responseStanza, String nodeName,
																 Collection<RetrieveItemsAsyncCallback.Item> items) {
										   if (shouldReceive) {
											   assertEquals("Could not retrieve metadata info", 1, items.size());
											   for (Item item : items) {
												   try {
													   Element info = item.getPayload().getChildren("info").get(0);
													   assertEquals("Received metadata info with wrong bytes value",
																	String.valueOf(data.length),
																	info.getAttribute("bytes"));
													   assertEquals("Received metadata info with wrong id", id,
											   ...
											   assertEquals(
													   "Was able to retrieve metadata but it should not be allowed", 0,
													   items.size());
										   }
									   }
		...
		mutex.waitFor(1000 * 20,
					  "received:meta:" + id + ":" + jaxmpp.getSessionObject().getUserBareJid().toString() + ":" + uuid);
		Thread.sleep(1000 * 2);
		assertTrue("Error during retrieval of metadata item", mutex.isItemNotified(
				"received:meta:" + id + ":" + jaxmpp.getSessionObject().getUserBareJid().toString() + ":" + uuid));
		...
		pubsubModule.retrieveItem(userJid, USER_AVATAR_DATA_NODE, id, new RetrieveItemsAsyncCallback() {
			...
			protected void onEror(IQ response, XMPPException.ErrorCondition errorCondition,
				...
				if (!shouldReceive) {
					mutex.notify(
							"received:data:" + id + ":" + jaxmpp.getSessionObject().getUserBareJid().toString() + ":" +
									uuid);
				} else {
		...
		mutex.waitFor(1000 * 20,
					  "received:data:" + id + ":" + jaxmpp.getSessionObject().getUserBareJid().toString() + ":" + uuid);
		Thread.sleep(1000 * 2);
		assertTrue("Error during retrieval of data item", mutex.isItemNotified(
				"received:data:" + id + ":" + jaxmpp.getSessionObject().getUserBareJid().toString() + ":" + uuid));
	}
}
